...
class report_account_aged_forward_partner(models.AbstractModel):
    _name = "account.aged.forward.partner"
    _description = "Aged Forward Partner Balances"
    ...
    def _get_lines(self, options, line_id=None):
        ...
        results, total, amls = self.with_context(**context)._get_partner_move_lines_forward(account_types, self._context['date_to'], 'posted', 30)
    ...
    def _get_partner_move_lines_forward(self, account_type, date_from, target_move, period_length):
        # Amended from account/report/account_aged_partner_balance.py
        ...
        # PCV: In case of a period_length of 30 days as of 2020-08-27, we want the following periods:
        # Name       Stop         Start
        # 1 - 30   : 2020-08-28 - 2020-09-26
        # 31 - 60  : 2020-09-27 - 2020-10-26
        # 61 - 90  : 2020-10-27 - 2020-11-25
        # 91 - 120 : 2020-11-26 - 2020-12-25
        # +120     : 2020-12-26
        ...
        for i in range(5)[::-1]:
            ...
            period_start = (start + relativedelta(days=1)).strftime('%Y-%m-%d')
            period_stop = (i!=0 and (stop).strftime('%Y-%m-%d') or False)
            if i == 0:
...
class report_account_aged_forward_receivable(models.AbstractModel):
    _name = "account.aged.forward.receivable"
    _description = "Aged Forward Receivable"
    _inherit = "account.aged.forward.partner"

    def _set_context(self, options):
        ctx = super(report_account_aged_forward_receivable, self)._set_context(options)
        ctx['account_type'] = 'receivable'
        return ctx

    def _get_report_name(self):
        return _("Aged Forward Receivable")

    def _get_templates(self):
        templates = super(report_account_aged_forward_receivable, self)._get_templates()
        templates['line_template'] = 'account_reports.line_template_aged_receivable_report'
        return templates


class report_account_aged_forward_payable(models.AbstractModel):
    _name = "account.aged.forward.payable"
    _description = "Aged Forward Payable"
    _inherit = "account.aged.forward.partner"

    def _set_context(self, options):
        ctx = super(report_account_aged_forward_payable, self)._set_context(options)
        ctx['account_type'] = 'payable'
        ctx['aged_balance'] = True
        return ctx

    def _get_report_name(self):
        return _("Aged Forward Payable")

    def _get_templates(self):
        templates = super(report_account_aged_forward_payable, self)._get_templates()
