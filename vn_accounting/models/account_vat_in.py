...
class vat_in_report(models.AbstractModel):
    ...
    def _get_taxs_by_aml(self, aml_id):
        self._cr.execute("""SELECT account_tax.amount, account_tax.amount_type,
                ...
                WHERE account_move_line_account_tax_rel.account_move_line_id = """+ str(aml_id) +"""
                """)
        taxs = self._cr.dictfetchall()
        return taxs

    
    def _get_aml_by_vat_categ(self, options, vat_categ):
        company_id = self.env.user.company_id
        #cond = [options['date']['date_from'],options['date']['date_from'], vat_categ.id, options['date']['date_to'], options['date']['date_to'], company_id.id]
        cond = [options['date']['date_from'], vat_categ.id, options['date']['date_to'], company_id.id]
        inv_cond = """"""
        ...
        self._cr.execute("""
            ...
            aml.partner_id, aml.date, aml.product_id, ai.vat_invoice_no as vat_invoice_number, aml.tax_line_id as tax,
                CASE WHEN ai.type in ('out_refund', 'in_refund')  OR aml.ref like 'Reversal of%%'
            ...
            WHERE aml.product_id is not null and (aml.date_accounting >= %s) and aml.vat_in_config_id = %s and
                (aml.date_accounting <= %s) and aml.company_id = %s and am.state = 'posted'  """+ partner_cond + inv_cond + vatp_cond +"""
    ...
    def get_all_total(self, options, line_id, type='purchase'):
        ...
        for vat_categ in vat_categs:
            results = self._get_aml_by_vat_categ(options, vat_categ)
            for am in results:
                tax_rate = 0
                taxed = 0
    ...
    def _group_by_vat_categ(self, options, line_id):
        vatcategs = {}
        vat_categs = self.env['vat.in.configuration'].sudo().search([])
    ...
    def _get_lines(self, options, line_id=None):
        ...
        grouped_vat = self.with_context(date_from_aml=dt_from, date_from=dt_from or None)._group_by_vat_categ(options, line_id)
        ...
        for vat in grouped_vat:
            ...
            if 'vat_%s' % (vat.id,) in options.get('unfolded_lines') or unfold_all:
                ...
                for am in amls:
                    ...
                    columns = [{'name': v} for v in [invoice_vat, invoice_date, partner_name, partner_vat, line.product_id.name, self.format_value(am.get('tax_amount')), tax_rate, self.format_value(taxed), line.invoice_id.note ]]
        ...
        if not line_id:
            lines.append({
                    ...
                    'columns': [{'name': v} for v in ['', '', '', '', '', self.format_value(total.get('global_base')), self.format_value(total.get('global_tax_rate')),self.format_value(total.get('global_taxed')),'']],
                    'level': 2,
                })
        return lines
    ...
    def get_xlsx(self, options, response):
        ...
        sheet.write(y+1, 0, 'Tổng số thuế GTGT của hàng hoá, dịch vụ mua vào : '+str(amount_with_tax), header_left_normal)
