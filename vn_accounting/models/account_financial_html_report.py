...
class AccountFinancialHtmlReport(models.Model):
	...
	def _can_get_info(self, options):
		if self.id == self.env.ref('vn_accounting.account_financial_report_pnl_b02').id:
			if options:
	...
	def get_report_informations(self, options):
		...
		if self.id in [self.env.ref('vn_accounting.account_financial_report_line_b01_01_02_01_01').id, self.env.ref('vn_accounting.account_financial_report_pnl_b02').id]:

			bs_report = self.env.ref('vn_accounting.account_financial_report_b01')
			pnl_report = self.env.ref('vn_accounting.account_financial_report_pnl_b02')
			
			if self.id == self.env.ref('vn_accounting.account_financial_html_report_action_%s' % (bs_report.id,)).id:
				default_filter = 'this_year'
			elif self.id == self.env.ref('vn_accounting.account_financial_html_report_action_%s' % (pnl_report.id,)).id:
			...
			if comparison and comparison.get('number_period') in [1,0]:
	...
	# 	vn_report = self.env.ref('vn_accounting.account_financial_report_pnl_b02')
	...
	def _get_report_dict(self, options):
		bs_report = self.env.ref('vn_accounting.account_financial_report_b01')
		icf_report = self.env.ref('vn_accounting.account_financial_report_icf_b03')
	...
	def get_html(self, options, line_id=None, additional_context=None):
		...
		self = self.with_context(lang='vi_VN')
		# only for bs b01 dn
		customized_reports = self.env.ref('vn_accounting.account_financial_report_b01')
		if self.id in customized_reports.ids:
			self = self.with_context(selected_line_id=line_id)
		
		pl_report = self.env.ref('vn_accounting.account_financial_report_pnl_b02')
		bs_report = self.env.ref('vn_accounting.account_financial_report_b01')
		icf_report = self.env.ref('vn_accounting.account_financial_report_icf_b03')
		...
		if self == pl_report:
			...
			if self.env.context.get('print_mode', False):
				...
				if self.env.ref('vn_accounting.account_financial_report_pnl_b02').id == self.id:
	...
	def set_xlsxb01_body(self, options, workbook, sheet):
		...
		for line in lines:
			...
			closing = columns[0]['no_format_name']

			opening = ''
			if len(columns) > 1:
				opening = columns[1]['no_format_name']
	...
	def set_xlsxb03_body(self, options, workbook, sheet):
		...
		for line in lines:
			...
			this_value = ''
			if columns[0]['name'] != '':
				this_value = columns[0]['no_format_name']

			prev_value = ''
			if len(columns) > 1 and columns[1]['name'] != '':
				prev_value = columns[1]['no_format_name']
...
class AccountFinancialHtmlReportLine(models.Model):
	...
	def _get_lines(self, financial_report, currency_table, options, linesDicts):
		...
		customized_reports = self.env.ref('vn_accounting.account_financial_report_b01') + self.env.ref('vn_accounting.account_financial_report_pnl_b02') + self.env.ref('vn_accounting.account_financial_report_icf_b03')
	...
	def _get_aml_domain(self):
		...
		result = (safe_eval(self.domain) or []) + (self._context.get('filter_domain') or []) + (self._context.get('group_domain') or [])
		...
		for line in lines:
			if line.get('code') == bs_code:
				...
				if isinstance(columns, list):
					bs_value = columns[0]['no_format_name']
				break
	...
	def _get_bs_01_dn_balance(self):
		""" Get value for Indirect Cash Flow from Balance Sheet as source """
		f_report = self.env.ref('vn_accounting.account_financial_report_b01')
...
class CustomFormulaLine(object):
	...
	def __init__(self, obj, currency_table, financial_report, type='balance', linesDict=None):
		...
		# self.balance = 1.1
		try:
			print(obj.code, self.balance)
		except:
			print(obj, self.balance)
