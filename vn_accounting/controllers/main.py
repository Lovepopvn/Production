    ...
    def get_report(self, model, options, output_format, token, financial_id=None, **kw):
        ...
        try:
            ...
            if output_format == 'xlsx':
                ...
                if report_obj.id == request.env.ref('vn_accounting.account_financial_report_pnl_b02').id:
                    # if PNL CP
                    # report.get_xlsx_pnl(options, response)
                    report_obj._get_xlsx_pnl(options, response)
                elif report_obj.id == request.env.ref('vn_accounting.account_financial_report_b01').id:
                    report_obj.with_context(lang='vi_VN').get_xlsxb01(options, response)
                elif report_obj.id == request.env.ref('vn_accounting.account_financial_report_icf_b03').id:
