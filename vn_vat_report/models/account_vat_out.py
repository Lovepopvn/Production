...
class vat_out_report(models.AbstractModel):
    ...
    filter_tax_name = False
    filter_display_invoice_number = True
    filter_display_tax_name = True
    ...
    def _get_amlids_by_tax(self, tax_ids):
    ...
    def _get_inv_cond(self, options):
        inv_cond = ''
        if options.get('invoice_number'):
            inv_cond = f"AND am.name ILIKE '%{options['invoice_number']}%'"
        return inv_cond

    def _get_partner_cond(self, options):
        partner_cond = ''
        if options.get('partner_ids'):
            partner_ids_str = ','.join([str(partner_id) for partner_id in options['partner_ids']])
            partner_cond = f"AND aml.partner_id IN ({partner_ids_str})"
        return partner_cond

    def _get_vatp_cond(self, options):
        vatp_cond = ''
        if options.get('vat_number'):
            vatp_cond = f"AND rp.vat ILIKE '%{options['vat_number']}%'"
        return vatp_cond

    def _get_aml_cond(self, tax_ids):
        list_aml = self._get_amlids_by_tax(tax_ids)
        aml_cond = 'AND aml.id is NULL'
        if list_aml:
    ...
    def _get_aml_by_tax(self, options, tax_id):
        ...
        aml_cond = self._get_aml_cond(tax_id)
        ...
        sql = f"""SELECT aml.id as id,
                         ...
                         CASE WHEN am.type in ('out_refund', 'in_refund') THEN -aml.countered_amt
    ...
    def _get_lines(self, options, line_id=None):
        ...
        grouped_taxes = self.with_context(date_from_aml=dt_from, date_from=dt_from or None)._group_by_tax_id(options,
                                                                                                             line_id,
                                                                                                             'sale')
                ...
                for am in amls:
                    ...
                    columns = [{'name': v} for v in
                               [invoice_vat, invoice_date, partner_name, partner_vat, line.product_id.name,
                                self.format_value(am.get('tax_amount')), self.format_value(taxed),
                                line.move_id.note]]
    ...
    def get_xlsx(self, options, response=None):
        ...
        sheet.write(y + 1, 0, 'Tổng doanh thu hàng hoá, dịch vụ bán ra chịu thuế GTGT : ' + str(amount_with_tax),
                    header_left_normal)
