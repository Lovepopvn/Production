...
class LogPlugin(Plugin):
    ...
    def egress(self, envelope, http_headers, operation, binding_options):
        self.debug_logger(envelope, 'fedex_shipping_request')
...
class FedexRequest():
    ...
    def __init__(self, debug_logger, request_type="shipping", prod_environment=False, ):
        ...
        if request_type == "shipping":
            if not prod_environment:
                wsdl_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../api/test/ShipService_v21.wsdl')
            else:
                wsdl_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../api/prod/ShipService_v21.wsdl')
    ...
    def set_shipper(self, company_partner, warehouse_partner):
        ...
        if warehouse_partner.street2:
            self.RequestedShipment.Shipper.Address.StreetLines += [warehouse_partner.street2, ]
        # else:
        #     assert warehouse_partner.street2 is None

        self.RequestedShipment.Shipper.Address.City = warehouse_partner.city
        self.RequestedShipment.Shipper.Address.PostalCode = warehouse_partner.zip
        if warehouse_partner.country_id:
            self.RequestedShipment.Shipper.Address.CountryCode = warehouse_partner.country_id.code
        # else:
        #     assert warehouse_partner.country_id is None

        if warehouse_partner.state_id:
            self.RequestedShipment.Shipper.Address.StateOrProvinceCode = warehouse_partner.state_id.code
        # else:
        #     assert warehouse_partner.state_id is None
    ...
    def set_recipient(self, recipient_partner):
        ...
        # taxpayer_identification = shipment.create_wsdl_object_of_type('TaxpayerIdentification')
        # if tax_number:
        #     taxpayer_identification.Number = tax_number
        #     taxpayer_identification.TinType = "BUSINESS_NATIONAL"
        # else:
        #     assert tax_number is None  # pre

        # shipment.RequestedShipment.Recipient.Tins = [taxpayer_identification, ]

        if recipient_partner.street2:
            self.RequestedShipment.Recipient.Address.StreetLines += [recipient_partner.street2, ]
        # else:
        #     assert recipient_partner.street2 is None

        self.RequestedShipment.Recipient.Address.City = recipient_partner.city

        if recipient_partner.state_id:
            self.RequestedShipment.Recipient.Address.StateOrProvinceCode = recipient_partner.state_id.code
        # else:
        #     assert recipient_partner.state_id is None

        self.RequestedShipment.Recipient.Address.PostalCode = recipient_partner.zip
        if recipient_partner.country_id:
            self.RequestedShipment.Recipient.Address.CountryCode = recipient_partner.country_id.code
        # else:
        #     assert recipient_partner.country_id is None
    ...
    def add_package(self, weight_value, package_code=False, package_height=0, package_width=0, package_length=0, sequence_number=False, mode='shipping'):
        ...
        return self._add_package(weight_value=weight_value, package_code=package_code, package_height=package_height, package_width=package_width,
                                 package_length=package_length, sequence_number=sequence_number, mode=mode, po_number=False, dept_number=False, inv_number=False)

    def _add_package(self, weight_value, package_code=False, package_height=0, package_width=0, package_length=0, sequence_number=False, mode='shipping', po_number=False, dept_number=False, reference=False, inv_number=False):
        ...
        if inv_number:
            inv_reference = self.factory.CustomerReference()
            inv_reference.CustomerReferenceType = 'INVOICE_NUMBER'
            inv_reference.Value = inv_number
            package.CustomerReferences.append(inv_reference)
        if po_number:
    ...
    def shipment_label(self, label_format_type, image_type, label_stock_type, label_printing_orientation, label_order):
        ...
        LabelSpecification.CustomerSpecifiedDetail.DocTabContent = self.factory.DocTabContent()
        LabelSpecification.CustomerSpecifiedDetail.DocTabContent.DocTabContentType = "STANDARD"
    ...
    def commercial_invoice(self, document_stock_type, invoice_num, send_etd=False):
        ...
        self.RequestedShipment.CustomsClearanceDetail.CommercialInvoice.CustomerReferences.append(
    ...
    def shipping_charges_payment(self, shipping_charges_payment_account, payment_type):
    ...
    def duties_payment(self, sender_party, responsible_account_number, payment_type):
        ...
        if payment_type in ('SENDER','THIRD_PARTY'):
            Payor = self.factory.Payor()
            Payor.ResponsibleParty = self.factory.Party()
    ...
    def customs_value(self, customs_value_currency, customs_value_amount, document_content, invoice_num):
        ...
        # self.RequestedShipment.PreferredCurrency = self.factory.PreferredCurrency()
        self.RequestedShipment.TotalInsuredValue = self.factory.Money()

        # self.RequestedShipment.PreferredCurrency = customs_value_currency
        self.RequestedShipment.TotalInsuredValue.Currency = customs_value_currency
        self.RequestedShipment.TotalInsuredValue.Amount = customs_value_amount
